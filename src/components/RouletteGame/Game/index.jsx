...
function RouletteGame() {
  ...
  const resetLayout = async() => {
    ...
    setHighLow([0, 0]);
    if(!isSpinComplete) {
      await updateChipCount(chipCount + pendingTotalBet)
    }
  ...
  const undoRecentBet = async () => {
    ...
    await updateChipCount(chipCount+recentBetValue)
  ...
  const collectWinnings = async () => {
    setIsSpinComplete(false)
    if(totalAmountWon > 0) {
      await updateChipCount(chipCount + totalAmountWon)
      setChipCount(chipCount+totalAmountWon)  
    }else{
        await updateChipCount(chipCount)
    }
    console.log('total amount', totalAmountWon)
    console.log('pending pay', pendingTotalBet)
    console.log('entrou collect')
    resetLayout()
  }
