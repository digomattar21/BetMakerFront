...
export const BetCard = ({ h2hOdd,id }) => {
  const classes = useStyles();
  const [spreads, setSpreads] = useState()
  const [totals, setTotals] = useState();
  const [mainOdds,setMainOdds] = useState([]);
  ...
  const displayOdds = () => {
    if (h2hOdd) {
      let homeTeamOddsArray = []
      let awayTeamOddsArray = []
      let drawOddsArray=[];
      let sites = h2hOdd.sites;
      sites.forEach((site,index)=>{
        if (site.odds.h2h.length>2){
          homeTeamOddsArray.push(site.odds.h2h[0])
          awayTeamOddsArray.push(site.odds.h2h[1])
          drawOddsArray.push(site.odds.h2h[2])
        }else{
          homeTeamOddsArray.push(site.odds.h2h[0])
          awayTeamOddsArray.push(site.odds.h2h[1])
        }
      ...
      if (drawOddsArray.length>0){
        let homeOddAvg = (((homeTeamOddsArray.reduce((acc,curr)=>acc+curr))/homeTeamOddsArray.length)).toFixed(2);
        let awayOddAvg = (((awayTeamOddsArray.reduce((acc,curr)=>acc+curr))/awayTeamOddsArray.length)).toFixed(2);
        let drawOddAvg = (((drawOddsArray.reduce((acc,curr)=>acc+curr))/drawOddsArray.length)).toFixed(2);
        setMainOdds([homeOddAvg,awayOddAvg,drawOddAvg])
        
      }else{
        let homeOddAvg = (((homeTeamOddsArray.reduce((acc,curr)=>acc+curr))/homeTeamOddsArray.length)).toFixed(2);
        let awayOddAvg = (((awayTeamOddsArray.reduce((acc,curr)=>acc+curr))/awayTeamOddsArray.length)).toFixed(2);
        setMainOdds([homeOddAvg,awayOddAvg])
  ...
  return (
    <Card className={classes.root}>
      <CardContent style={{display:'flex', justifyContent: 'center'}}>
        <div className={classes.teamsContainer}>
          <div className={classes.teamsSubContainer}>
            ...
            <h2 style={{marginLeft:'7px'}}>{h2hOdd.teams[0]}</h2>
          </div>
          <div>
            <h2>X</h2>
          </div>
          <div className={classes.teamsSubContainer}>
            <h2 style={{marginRight:'7px'}}>{h2hOdd.teams[1]}</h2>
      ...
      <CardActions className={classes.cardActions}>
        ...
        >
          <Button  onClick={()=>handleOddClick(mainOdds[0], h2hOdd)}>{mainOdds[0] && <h3>{mainOdds[0]}</h3>}</Button>
          <Button  onClick={()=>handleOddClick(mainOdds[2], h2hOdd)}>{mainOdds[2] && <h3>{mainOdds[2]}</h3>}{!mainOdds[2] && <h3>X</h3>}</Button>
          <Button  onClick={()=>handleOddClick(mainOdds[1],h2hOdd)}>{mainOdds[1] && <h3>{mainOdds[1]}</h3>}</Button>
        </ButtonGroup>
      </CardActions>
    </Card>
